...
> {
  ...
  getCheckStatus(filteredDataSource: TransferItem[]) {
    ...
    if (checkedKeys.length === 0) {
      ...
      filteredDataSource.every(item => checkedKeys.indexOf(item.key) >= 0)
    ) {
      return 'all';
    }
    return 'part';
  ...
  render() {
    ...
    const listBody = bodyDom || (
      ...
      >
        ...
        <Animate
          ...
          transitionName={
            this.state.mounted ? `${prefixCls}-content-item-highlight` : ''
          }
    ...
    const checkAllCheckbox = (
      <Checkbox
        ...
        onChange={() =>
          this.props.handleSelectAll(filteredDataSource, checkedAll)
        }
      />
    );

...
class JDatePicker extends React.Component<any, any> {
    ...
    getLimitDate = (currentDate: any, limitDate: any) => {
        ...
        let isDateAfter = currentDate.isAfter(limitDate.format('YYYY-MM-DD'), 'day');
        ...
        return {
            disabledDate: (current:any) => {
                return current&&current.isBefore(limitDate.format('YYYY-MM-DD'), 'day');
            },
            disabledTime: () => {
                return {
                    disabledHours: () => TIME_LIST.slice(0, limitHour),
                    disabledMinutes: () => TIME_LIST.slice(0, limitMinute),
                };
            },
        };
    };
    render() {
        let { limitTime, getRef, onChange, ...rest } = this.props;
        let { current } = this.state;
        let limitProps = limitTime ? this.getLimitDate(current, limitTime) : {};
        return (
            <DatePicker

...
> {
  ...
  componentDidUpdate(preProps: any) {
    const { tabActiveKey }: any = this.props;
    if (preProps.tabActiveKey !== tabActiveKey) {
      this.getBreadcrumbDom();
    }
  }
  onChange = (key: any) => {
    const { onTabChange }: any = this.props;
    if (onTabChange) {
      onTabChange(key);
    }
  };
  ...
  getBreadcrumbDom = () => {
    const breadcrumb: any = this.conversionBreadcrumbList();
    this.setState({
      breadcrumb,
    });
  };
  ...
  conversionFromLocation = (routerLocation: any, breadcrumbNameMap: any) => {
    const { breadcrumbSeparator, linkElement = 'a' }: any = this.props;
  ...
  conversionBreadcrumbList = () => {
    const { breadcrumbList, breadcrumbSeparator }: any = this.props;
    const {
      routes,
      params,
      routerLocation,
  ...
  itemRender = (route: any, params: any, routes: any, paths: any) => {
    const { linkElement = 'a' }: any = this.props;

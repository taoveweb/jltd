...
const ReportConfigList = ({
  ...
  function returnInfoList(list: any, tip: any) {
    const selectList = list.filter((info: any) => {
    ...
    return selectList.map((info: any) => {
  ...
  const operateButton: any = [
    ...
    },
    {
      ...
      onClick: () => {
        ...
        if (!_flag) return;
        const infoList = selectList.map((info: any) => {
          const { id, recVer } = info;
          return { id, recVer };
        });
        dispatch({
...
export default connect(({ reportConfigList, loading, dictByCode }: any) => ({
  reportConfigList,
  loading,
  dictByCode,
}))(ReportConfigList);

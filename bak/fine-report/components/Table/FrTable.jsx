...
export default class FrTableComponents extends React.Component {
    constructor(props) {
        super(props);
        this.sendMessage = () => {
            const cb = (json) => {
                ...
                if (json) {
                    ...
                    if (j.beforetoexcel) {
                        ...
                        if (this.frm_dom) {
                            ...
                            this.setState({
                                pagination: Object.assign({}, this.state.pagination, { pageSize: pagesize, pageSizeOptions: [pagesize + ''], current: j.afterload.currentPageIndex, total: pagesize * j.afterload.reportTotalPage }),
                            });
                        }
                    }
                }
        ...
        this.onSearchClick = (v) => {
            if (!this.frm_dom)
                return;
            if (v) {
                let c = Object.assign({}, this.frm_param, v);
    ...
    render() {
        return (<div>
        <div className="page-body ant-table" ref={div => {
            this.iframe_div = div;
        }} style={{ margin: '16px 0', overflowY: 'hidden', overflowX: 'scroll' }}>
          <iframe style={{ overflow: 'hidden' }} ref={iframe => {
            this.frm_dom = iframe;
        }} src={this.frm_url} width="100%" height="0" scrolling="no" frameBorder="0"/>
        </div>
        <div style={{ float: 'right' }}>
          <Pagination showSizeChanger showQuickJumper hideOnSinglePage {...this.state.pagination} onChange={(page) => {
            window.sendMessage('_g().gotoPage(' + page + ')', 
            // @ts-ignore
...
FrTableComponents.printReport = (ifr, reportCode, printParam) => {
    // @ts-ignore
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data) {
                if (data.success) {
                    var s = 'FR.doURLPDFPrint({url : "' + data.data + '",isPopUp : false})';

...
class Detail extends React.Component {
    ...
    componentWillReceiveProps(nextPorps) {
        if (this.props.reportConfigList.currentItem !==
    ...
    // }
    render() {
        const { dispatch, loading, reportConfigList, dictByCode, form, form: { getFieldsValue, validateFields }, } = this.props;
        const { currentItem } = reportConfigList;
        const { moduleList } = dictByCode;
        const { reportCode, reportName, reportType, isTenant, publishStatus, reletiveUrl, paramJson, remark, recVer, id, } = currentItem;
        // TODO缺少数据信息
        ...
        const onSave = () => {
            validateFields((errors) => {
                ...
                const data = Object.assign({}, getFieldsValue(), { recVer: recVer, id: id });
                dispatch({
        ...
        // };
        return (<PageHeaderLayout title="报表模板编辑" showBackBtn>
        <div>{renderControllerList}</div>
        
        <Button type="primary" onClick={onSave} loading={loading.effects['reportConfigList/save']}>
          保存
        </Button>
      </PageHeaderLayout>);
    }
...
})(connect(({ loading, reportConfigList, dictByCode }) => ({
    loading,
    reportConfigList,
    dictByCode,
}))(Detail));

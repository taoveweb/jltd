...
//import { buttonGroup, LabelWithController, Table } from 'components';
require('./List.less');
const List = ({ customCommon, loading, dispatch, form, }) => {
    const { customDetailPage, selectedRowKeys } = customCommon;
    const onChange = (text, definedId, key) => {
        let index = null;
        customDetailPage.some((item, indexTemp) => {
    ...
    const renderInput = (title, text, definedId, key, rules) => {
        const info = {
            id: `${definedId}-${key}`,
            label: title,
            type: 'input',
    ...
    const columns = [
        ...
        },
        {
            ...
            key: 'extendProperty',
            width: 250,
            render: (text, rowData) => renderInput('自定义扩展属性(JSON格式)', text, rowData.definedId, 'extendProperty', [{ max: 800 }]),
        },
    ];
    const getIndex = (list, value) => {
        let selectIndex;
        list.some((info, index) => {
            if (info.id === value) {
                selectIndex = index;
                return true;
            }
    ...
    const operateButton = [
        ...
        },
        {
            ...
            onClick: () => {
                ...
                Modal.confirm({
                    ...
                    onOk: () => {
                        const newCustomDetailPage = customDetailPage.filter((info) => {
        ...
        },
        {
            ...
            onClick: () => {
                ...
                selectedRowKeys.forEach((selectKey) => {
                    const selectIndex = getIndex(newCustomDetailPage, selectKey);
    ...
    const idList = customDetailPage.map((info) => info.definedId || info.id);
    return (<div>
      <div className="menu-btn-group">{buttonGroup(operateButton)}</div>
      <Table 
    // @ts-ignore
    dataSource={customDetailPage} pagination={false} loading={loading.effects['customCommon/getCustomDetailPage']} className={classnames('jlt-mt-20')} columns={columns} rowKey={(record) => record.id} rowSelection={{
        selectedRowKeys,
        onChange: (keys) => {
            const sortKeys = [];
            customDetailPage.forEach((info) => {
...
export default connect(({ loading, customCommon }) => ({
    customCommon,
    loading,
}))(List);

  ...
  render() {
    ...
    const onOk = (data: any) => {
      ...
      dispatch({
        ...
        payload: {
          currentItem: { tenantId: id, ...currentItem },
        },
      });
    ...
    const onOkRpt = (data: any) => {
      ...
      dispatch({
        type: 'tenantReportConfigList/updateState',
        payload: {
          currentItem: { reportCode, ...currentItem },
        },
      });
      // this.setState({ reportCode: reportCode });
    };
    return (
      <PageHeaderLayout title="租户定制报表模板" showBackBtn>
        <div>{renderControllerList}</div>
        <Button
          type="primary"
          onClick={onSave}
          loading={loading.effects['tenantReportConfigList/save']}
        >
          保存
        </Button>
...
export default Form.create({
  ...
  connect(({ loading, tenantReportConfigList }: any) => ({
    loading,
    tenantReportConfigList,
  }))(Detail),
);

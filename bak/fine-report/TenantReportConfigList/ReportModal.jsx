...
const ReportModal = ({ dispatch, loading, reportConfigList, onCancel, onOk, }) => {
    const { list, pagination, selectedRowKeys, reportConfigVisible, } = reportConfigList;
    const handleRefresh = (newQuery) => {
        dispatch({
            type: 'reportConfigList/query',
            payload: Object.assign({ isTenant: 1 }, newQuery),
        });
        dispatch({
    ...
    // TODO:宽
    return (<Modal title={'报表模板列表'} destroyOnClose visible={reportConfigVisible} onOk={handleOk} onCancel={onCancel} width={920}>
...
export default connect(({ loading, reportConfigList }) => ({
    loading,
    reportConfigList,
}))(ReportModal);
